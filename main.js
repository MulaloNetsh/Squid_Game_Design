...
class ThirdPersonCameraDemo {
  ...
  _Initialize() {
    ...
    this._camera.position.set(100, -100, 0);
        ...
        this.cameraTop.position.set(0,100,90);
    ...
    const bgTexture = bgdloader.load('bground.jpg');
...
loadSoldier() {
  const loader = new GLTFLoader();
  loader.load("../models/soldier/scene.gltf", (gltf) => {
      gltf.scene.traverse(c => {
          c.castShadow = true;
      });
      gltf.scene.position.set(20, -10,-210);
...
lookBackward() {
    gsap.to(this.doll.rotation, { y: -3.15, duration: 3 })
}
lookForward() {
